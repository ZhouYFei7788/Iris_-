...
def insert_model_results(model_name, mean_score, std_score, epoch_loss=None, epoch=None):
    # 确保epoch_loss是有效的数值
    epoch_loss = float(epoch_loss) if epoch_loss is not None and not np.isnan(epoch_loss) else None
    cursor = conn.cursor()
    ...
    # 使用双引号括起中文列名
    sql = """
        INSERT INTO 综合信息 (记录ID, 模型名称, 评估平均准确率, "评估准确率标准差", "训练损失", "训练周期")
        VALUES (:record_id, :model_name, :mean_score, :std_score, :epoch_loss, :epoch)
    """
    cursor.execute(sql, record_id=next_record_id, model_name=model_name,
